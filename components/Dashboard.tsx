  ...
  return (
    <div className="max-w-6xl mx-auto space-y-16 py-8">
      <header className="text-center space-y-6">
        <h2 className="text-6xl md:text-7xl font-serif-ornate font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-200 via-yellow-500 to-amber-700 drop-shadow-[0_5px_15px_rgba(0,0,0,0.5)]">
          L'Oracle Antique
        </h2>
        <div className="flex items-center justify-center gap-4">
          <div className="h-[1px] w-24 bg-gold/50"></div>
          <p className="text-2xl font-cursive text-amber-100/80">Entrez dans la pénombre du savoir millénaire...</p>
          <div className="h-[1px] w-24 bg-gold/50"></div>
        </div>
      </header>
...
const AntiquePlate: React.FC<{ title: string; description: string; icon: string; onClick: () => void }> = ({ title, description, icon, onClick }) => (
  ...
  >
    ...
    <h4 className="text-2xl font-serif-ornate font-bold mb-4 tracking-widest uppercase text-amber-200 group-hover:text-gold transition-colors">{title}</h4>
